%h5gOFDMDemodulate 3GPP 5G OFDM demodulation
%   GRID = h5gOFDMDemodulate(GNB,WAVEFORM) performs the inverse of the OFDM
%   modulation operation of <a href="matlab: help('h5gOFDMModulate')">h5gOFDMModulate</a>. It demodulates the
%   multi-antenna time domain waveform WAVEFORM to return the received
%   resource element array GRID. It is interface compatible with 
%   <a href="matlab: doc('lteOFDMDemodulate')">lteOFDMDemodulate</a> and
%   extends it to support reception on the DC subcarrier, total resource
%   blocks above 110 and NR subcarrier spacings above 15kHz (15*2^n kHz
%   for non-negative n). In addition to basic windowed CP-OFDM, it also
...
%   be the same as used in the h5gOFDMModulate modulator function for the
...
%   subframe. For an LTE baseline waveform, frame/subframe alignment can be
%   achieved by using the <a href="matlab: help('lteDLFrameOffset')">lteDLFrameOffset</a> function which relies on the 
%   waveform containing LTE synchronization and reference signals.
%
%   GRID = h5gOFDMDemodulate(GNB,WAVEFORM,CPFRACTION) allows the
...
%   of windowing of the <a href="matlab: help('h5gOFDMModulate')">h5gOFDMModulate</a> function.
%
%   Examples: 
%   % Demodulate an LTE Test Model 1.1 5MHz waveform.
%   cfg = lteTestModel('1.1','5MHz');
%   txWaveform = lteTestModelTool(cfg);
%   rxGrid = h5gOFDMDemodulate(cfg,txWaveform);
%   
%   % Demodulate a waveform created from a random RE subframe for 120 RB 
%   % and 30 kHz subcarrier spacing.
%   gnb = struct('NDLRB',120,'CyclicPrefix','Normal','SubcarrierSpacing',30);
...
%   waveform = h5gOFDMModulate(gnb,reGrid);
%   rxGrid = h5gOFDMDemodulate(gnb,waveform);
%   lteEVM(reGrid(:),rxGrid(:))
% 
%   See also h5gOFDMModulate, h5gOFDMInfo, lteOFDMModulate.

%   Copyright 2017 The MathWorks, Inc.

function reGrid = h5gOFDMDemodulate(gnb,waveform,varargin)
