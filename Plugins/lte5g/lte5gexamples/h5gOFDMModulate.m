%h5gOFDMModulate 3GPP 5G OFDM modulation
%   [WAVEFORM,INFO] = h5gOFDMModulate(...) performs OFDM waveform processing
%   on an input resource element grid including IFFT calculation, cyclic 
%   prefix insertion and optional windowing or filtering. It is interface
%   compatible with <a href="matlab: doc('lteOFDMModulate')">lteOFDMModulate</a> and extends it to support transmission
%   on the DC subcarrier, total resource blocks above 110 and NR subcarrier
%   spacings above 15kHz (15*2^n kHz for non-negative n). It is also 
%   capable of processing any number of OFDM symbols, where the index of 
%   the starting symbol of the input grid (relative to the 1ms subframes)
%   is defined optionally. In addition to basic windowed CP-OFDM, it also
%   supports W-OFDM (CP-OFDM with weighted overlap and add (WOLA)) and
%   F-OFDM (CP-OFDM with filtering) and the windowing or filtering is
%   applied in a cyclic fashion to give a loopable waveform at the output 
%   for test and measurement applications. In contrast to <a href="matlab: doc('lteOFDMModulate')">lteOFDMModulate</a>,
%   by default, the DC subcarrier is used for data transmission to reflect
%   NR agreements. Note that, at least up to 40 GHz for eMBB and URLLC
%   services, the CP-OFDM based waveform should support spectral 
%   utilization of Y greater than that of LTE (assuming Y=90% for LTE)
%   where Y (%) is defined as transmission bandwidth configuration / channel
%   bandwidth * 100%.
%
%   [WAVEFORM,INFO] = h5gOFDMModulate(GNB,GRID) performs OFDM modulation
...
%   documentation. The antenna planes in GRID are separately OFDM modulated
%   to give the columns of WAVEFORM. The number of resource blocks (NDLRB)
%   can exceed the LTE maximum of 110. The number of OFDM symbols in a
%   complete 1ms subframe will depend on the OFDM numerology defined by 
%   the subcarrier spacing and the cyclic prefix. Information about the
%   processing numerology is returned in the INFO structure (see <a href="matlab: doc('h5gOFDMInfo')">h5gOFDMInfo</a>).
%   If the input symbol grid does not align with 1ms subframes for the
%   associated numerology then the NSymbol parameter should be used to
%   specify the first symbol number associated with the grid data. 
% 
...
% 
%   For more details of the CP-OFDM processing, see <a href="matlab: doc('lteOFDMModulate')">lteOFDMModulate</a>.
%   For more details of the W-OFDM and F-OFDM processing, see R1-166999, 
%   "Detailed configuration of f-OFDM and W-OFDM for LLS evaluation", 
%   Spreadtrum Communications, RAN WG1 meeting #86.
%       
...
%   concatentation of the input grids before processing. The concatenation
...
%   h5gOFDMModulate. Any use of <a href="matlab: doc('h5gOFDMDemodulate')">h5gOFDMDemodulate</a> to recover the resource
%   elements should be time aligned with the individual waveform segments
%   created by h5gOFDMModulate. If it possible to create a continuous 
...
%
%   [WAVEFORM,INFO] = h5gOFDMModulate(GNB,GRID,WINDOWING) allows control of
%   the number of windowed and overlapped samples used in the time-domain
%   windowing. This applies to CP-OFDM only and the function signature is
%   provided for interface compatibility with <a href="matlab: doc('lteOFDMModulate')">lteOFDMModulate</a>.
%
%   Examples:
%   % CP-OFDM modulation of one LTE subframe of random uniformly-distributed 
%   % noise, using a 10MHz 2-antenna configuration.
%   
%   gnb = struct('NDLRB',50,'CyclicPrefix','Normal','CellRefP',2);
%   dims = lteDLResourceGridSize(gnb);
%   reGrid = complex(rand(dims)-0.5,rand(dims)-0.5);
%   [waveform,waveforminfo] = h5gOFDMModulate(gnb,reGrid);
% 
%   % W-OFDM 
%   gnb = struct('NDLRB',50,'CyclicPrefix','Normal','CellRefP',2);
%   gnb.WaveformType = 'W-OFDM';
%   gnb.Alpha = 0.1;
%   dims = lteDLResourceGridSize(gnb);
%   reGrid = complex(rand(dims)-0.5,rand(dims)-0.5);
%   [waveform,waveforminfo] = h5gOFDMModulate(gnb,reGrid);
%
%   % F-OFDM with 60kHz subcarrier spacing  
%   gnb = struct('NDLRB',50,'CyclicPrefix','Normal','CellRefP',2,'SubcarrierSpacing',60);
%   gnb.WaveformType = 'F-OFDM';
%   gnb.FilterLength = 513;
%   gnb.ToneOffset = 2.5;
%   waveinfo = h5gOFDMInfo(gnb);
%   dims = [waveinfo.NSubcarriers waveinfo.SymbolsPerSubframe 1];
%   reGrid = complex(rand(dims)-0.5,rand(dims)-0.5);
%   [waveform,waveforminfo] = h5gOFDMModulate(gnb,reGrid);
%
%   See also h5gOFDMDemodulate, h5gOFDMInfo, h5gOFDMDemodulate.

%   Copyright 2017 The MathWorks, Inc.

function [waveform,info] = h5gOFDMModulate(gnb,grid,varargin)
