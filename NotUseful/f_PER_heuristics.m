function nGoodPkt = f_PER_heuristics(candSet, problem, W, TXbits, MCS, channel_handles, array_handle)
...
for outer_iter = 1 : nUsers
    for inner_iter = 1 : nUsers
        response(outer_iter, inner_iter) =  patternAzimuth(handle_Conf_Array_USER{outer_iter}, ...
            centerfreq, problem.thetaUsers(inner_iter),'Azimuth', problem.phiUsers(inner_iter),'Type','power');
...
for outer_iter = 1 : nUsers
    ...
    if ~isempty(psdu_rx)
        nGoodPkt = nGoodPkt + double(~(any(biterr(psdu{outer_iter}, psdu_rx)) && rxflag));
    end
end
%------------- END CODE --------------
